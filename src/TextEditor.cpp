...
{
	SetPalette(GetBasicPalette());
	SetLanguageDefinition(LanguageDefinition::Basic());
			...
			else
			{
				...
				if (token_color == PaletteIndex::Identifier)
				{
					...
					else if (!line[first - bufferBegin].mPreprocessor)
					{
						...
						else if (mCustomClass.count(id) != 0)
							token_color = PaletteIndex::CustomClass;						
					}
				}
...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::Basic()
{
	...
	if (!inited)
	{
		...
		static const char* const keywords2[] = {
			...
			"KEY_W","KEY_X","KEY_Y","KEY_Z","KEY_LEFT_BRACKET","KEY_BACKSLASH","KEY_RIGHT_BRACKET","KEY_GRAVE",
			...
			"KEY_KP_4","KEY_KP_5","KEY_KP_6","KEY_KP_7","KEY_KP_8","KEY_KP_9","KEY_KP_DECIMAL","KEY_KP_DIVIDE",
		...
		static const char* const keywords3[] = {
			...
			"MOUSEWHEEL", "MOUSEPRESSED", "MOUSEDOWN", "MOUSERELEASED", "MOUSEUP", "MOUSESETPOS",
			"KEYPRESSED", "KEYDOWN", "KEYRELEASED", "KEYUP", "KEYGET", "KEYCHAR",
			"JOYISAVIABLE", "JOYNAME", "JOYPRESSED", "JOYDOWN",	"JOYRELEASED", "JOYUP", "JOYGET","JOYAXISCOUNT", "JOYAXISVALUE",			
			"FORMATTEXT", "FONTSPACE", "TEXTSIZE", "PEEK", "POKE", "SAVEDATA", "LOADDATA",
			"CHPRESET", "CHSET", "CHON", "CHOFF", "CHPLAY", "CHSTOP", "CHPOS", "CHSIZE", "CHFRAME", "CHGETNOTE", "CHGETNOTENUM"
		};
