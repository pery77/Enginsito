...
class blade
	...
	def draw()
		...
		oldY = positionY
		if direction = 0 then
			positionX = positionX - delta * speed * orientation
		else
			positionY = positionY - delta * speed * orientation
		endif
...
class ghost
	...
	def handleMove()
		if direction = 1 then
			backCol = hasColisionWithCachedTiles(collisionTiles,positionX - delta * speed + 1, positionY + 1, 14, 14)
			if not backCol then				
		...
		elseif direction = 3 then
			backCol = hasColisionWithCachedTiles(collisionTiles,positionX+ 1, positionY - delta * speed+ 1, 14, 14)
			if not backCol then
				if NOT (directionIndex = 3) then
					collisionTiles = getCollisionTiles(positionX, positionY, false, false)
					backCol = hasColisionWithCachedTiles(collisionTiles,positionX+ 1, positionY - delta * speed+ 1, 14, 14)
					if not backCol then
						positionY = positionY - delta * speed
					else
						randomDirection()
					endif
				else					
...
class character
	...
	def handleMove()
		if key_pressed(90) and not jumping then
			jumpCounter = 0
			jumping = 1
		endif
		...
		if key_down(65) or key_down(263) then
			backCol = hasColisionWithCachedTiles(collisionTiles,positionX - delta * speed +3, positionY+8, 10, 6)
			'backCol = checkWithBackground(positionX - delta * speed +3, positionY+8, 10, 6)
			if not backCol then				
			...
			directionIndex = 9
		elseif key_down(68) or key_down(262) then
			backCol = hasColisionWithCachedTiles(collisionTiles,positionX + delta * speed+3, positionY+8, 10, 6)
			'backCol = checkWithBackground(positionX + delta * speed+3, positionY+8, 10, 6)
			if not backCol then
			...
			directionIndex = 6
		elseif key_down(87) or key_down(265) then
			backCol = hasColisionWithCachedTiles(collisionTiles,positionX+3, positionY - delta * speed+8, 10, 6)
			'backCol = checkWithBackground(positionX+3, positionY - delta * speed+8, 10, 6)
			if not backCol then
				if NOT (directionIndex = 3) then
					collisionTiles = getCollisionTiles(positionX, positionY, false, false)
					backCol = hasColisionWithCachedTiles(collisionTiles,positionX+3, positionY - delta * speed+8, 10, 6)
			...
			directionIndex = 3
		elseif key_down(83) or key_down(264) then
			backCol = hasColisionWithCachedTiles(collisionTiles,positionX+3, positionY + delta * speed+8, 10, 6)
			'backCol = checkWithBackground(positionX+3, positionY + delta * speed+8, 10, 6)			
			if not backCol then
				if NOT (directionIndex = 0) then
...
def drawBottomTime()
	currentSeconds = currentSeconds + (delta / 1000)
...
def drawGame()
	...
	if  startCounter < 0.2 then
		startCounter = startCounter + (delta / 1000)
		return	
	endif
