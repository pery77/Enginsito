...
def draw()
    ...
    cls(0) 
    'Whit that we draw a text ("Hello World") in the position X = 104, and y = 40
    '2 is the size (the minimun is 1) and 3 is the color (white), colors go from 0 to 15
    draw.text("Hello World!",65,30,2,3) 
    'Another fancy text with usefull information.
    draw.text("Press ESCAPE key to close.",55,55,1,2)
    draw.text("ESCAPE",55 + 6*8,55,1,8)
enddef
